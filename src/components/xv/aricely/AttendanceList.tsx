...
const AttendanceList = () => {
	const { fetchInvite, currentInvite, deleteByName, loading } =
		useConfirmAttendance('aricely_lopez')
		...
		<section className="flex flex-col w-full">
			{(currentInvite?.guests?.length || 1) > 1 ? (
				<>
					...
					{/* <button
						className=" w-[90%]  ms:w-[85%] bg-red-950 text-white py-2 px-4 xs:text-sm ms:text-base text-center hover:shadow-lg transition ease-in-out mx-auto mb-8"
						onClick={onDownload}
					>
